...
class MassEmailBilling(models.TransientModel):
    ...
    wiz_type = fields.Selection([
        ('mail', 'Mail'),
        ('biostar', 'Biostar Update')], 'Type', default="mail", 
    ...
    def generate_bill(self):
        ...
        for rec in records:
            memObj = self.env['member.app'].browse([rec])
            if memObj:
                memObj.subscription_period = self.subscription_period
                memObj.calculate_overall_bill(self.subscription_period, True)
                memObj.update_period_payment_line()
                
                # for ls in memObj.mapped('summary_line'):
                #     unp_lines = ls.mapped('period_payment_line').filtered(lambda s: s.mark_paid == False)
                #     if unp_lines:
                #         value = len(unp_lines)
                #         ls.member_cost += ls.member_cost
                #         ls.spouse_cost += ls.spouse_cost
                #         ls.child_cost += ls.child_cost
                #             # values = {
                #             #     'subscription_period': self.subscription_period,
                #             #     'summary_line_id': ls.id,
                #             #     'mark_paid': False,
                #             #     'date_paid': False, # fields.Date.today(),
                #             #     }
                #             # ls.write({'period_payment_line': [(0, 0, values)]})
                            
                #     else:
                #         period_payment = memObj.create_period_payment(self.subscription_period, ls.id, False, False)
                #         ls.write({
                #             'period_payment_line': [(4, period_payment)],
                #         })
