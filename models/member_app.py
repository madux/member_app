...
class AbstractSectionLine(models.TransientModel):
    _name = "section.line.abstract"
    abstract_id = fields.Many2one('member.app')
    section_ids = fields.Many2one('section.product', string="Section")
    section_line_id = fields.Many2one('section.line', string="Section Line id")
    sub_payment_id = fields.Many2one('subscription.payment', string="Fee type")
    dependent_type = fields.Selection([
        ('member', 'Member'),
        ('spouse', 'Spouse'),
        ('child', 'Child'),
        ('guest', 'Guest'),
        ('relative', 'Relation')], string="Dependent type")
    amount = fields.Float('Amount')

    # def update_dependant_line(self):
    #     child_line = self.mapped('depend_name').filtered(lambda child: child.relationship == "Child").mapped('section_line')
    #     spouse_line = self.mapped('depend_name').filtered(lambda spouse: spouse.relationship != "Child").mapped('section_line')
    #     spouse_total = 0
    #     child_total = 0
    #     for record in spouse_line:
    #         spouse_total = record.amount
    #         summarylineids = self.mapped('summary_line').filtered(lambda summ: summ.section_ids.name == record.section_ids.name and summ.sub_payment_id.id == record.sub_payment_id.id)
    #         if summarylineids:
    #             summarylineids.update({'spouse_cost': spouse_total, 'child_cost': 0, 'member_cost': 0,})

    #     for rec in child_line:
    #         child_total = rec.amount
    #         summarylineids = self.mapped('summary_line').filtered(lambda summ: summ.section_ids.name == rec.section_ids.name and summ.sub_payment_id.id == rec.sub_payment_id.id)
    #         if summarylineids:
    #             summarylineids.update({'spouse_cost': 0, 'child_cost': child_total, 'member_cost': 0,})
...
class App_Member(models.Model):
    ...
    section_line_abstract = fields.One2many('section.line.abstract', 'abstract_id', string='Abstract Sections')
...
#  Depends on Periods
    ...
    def subscription_payment_lines(self, name, total_sum, amountpaid, pdate, advance_amount, outstanding, periods_month, payment_id, mark_paid=False):
        values = {
            'name': name,
            'member_id': self.id,
            'section_line_id': [(6, 0, [rec.id for rec in self.section_line])],
        ...
        self.write({'sub_line': [(0,0, values)]})
        self.exclude_one_time_fees()
    ...
    def regulate_payment(self):
        if not self.sub_line or self.summary_line:
    ...
    def exclude_one_time_fees(self):
        for rec in self.mapped('summary_line').filtered(lambda sef: sef.sub_payment_id.paytype in ['entry_fee', 'main_house']):
            if rec: 
                rec.write({
                        'one_time_fee': True,
                        'member_cost': 0,
            ...
            for check in check_lines:
                if not check.one_time_fee:
                    if bill_generate == False:
                        ...
                        'spouse_initial_amount': spouse_amountt if spouse_amountt > 0 else check.spouse_initial_amount,
                        })
                else:
                    check.write({
                        'member_cost': 0,
    ...
    def update_period_payment_line(self):
        '''
            Used to determine if the number of periods generated. if member have not made payment, the 
            system will update and add the left out payment to the respective column
        '''
        if self.summary_line:
            for sub in self.summary_line:
                if sub.period_payment_line:
                    unp_lines = sub.mapped('period_payment_line').filtered(lambda s: s.mark_paid == False)
                    if unp_lines:
                        for val in unp_lines:
                            sub.update({
                                'member_cost': sub.member_cost + sub.member_initial_amount if sub.member_cost >= 0 else 0,
                                'spouse_cost': sub.spouse_cost + sub.spouse_initial_amount if sub.spouse_cost >= 0 else 0,
                                'child_cost': sub.child_cost + sub.child_initial_amount if sub.child_cost >= 0 else 0,
                                'subscription_period': self.subscription_period,
                            })
                    
                period_obj = self.env['period.payment']
                period_obj.create({
                                    'subscription_period': self.subscription_period,
                                    'summary_line_id': sub.id,
                                    'mark_paid': False,
                                    'date_paid': fields.Date.today(),
                                    })

    def generate_member_summary_line(self, rec, subscription_period, sub_payment_id, section_ids,section_ids_name, amount,bill_generate):
        summary_line_obj = self.env['summary.section.line']
        check_lines = self.mapped('summary_line').filtered(lambda s: s.section_ids.name == section_ids_name and s.sub_payment_id.id == sub_payment_id)
        section_amount = self.env['section.line'].browse([rec]) 
        if check_lines:
            for check in check_lines:
                if not check.one_time_fee:
                    if bill_generate == False:
                        check.write({
                            'member_cost': amount if check.member_cost <= 0 else check.member_cost,
                            'spouse_cost': 0 if check.spouse_cost <= 0 else check.spouse_cost,
                            'child_cost': 0 if check.child_cost <= 0 else check.child_cost,
                            'member_initial_amount': amount,
                            # 'spouse_initial_amount': check.spouse_initial_amount + 0,
                            # 'child_initial_amount': check.spouse_initial_amount + 0,
                            })
    ...
    def update_dependant_line(self, bill_generate):
        child_line = self.mapped('depend_name').filtered(lambda child: child.relationship == "Child") 
        spouse_line = self.mapped('depend_name').filtered(lambda spouse: spouse.relationship != "Child") 
        spouse_total = 0
        child_total = 0
        summary_line_obj = self.env['summary.section.line']
        dependent_list = []
        for dep in spouse_line:
            for record in dep.section_line:
                abs_line = {'abstract_id': self.id, 'section_line_id': record.id, 'section_ids': record.section_ids.id, 'sub_payment_id': record.sub_payment_id.id, 'amount': record.amount, 'dependent_type': record.dependent_type}
                dependent_list.append(self.env['section.line.abstract'].create(abs_line).id)

        for dep in child_line:
            for record in dep.section_line:
                abs_line = {'abstract_id': self.id, 'section_line_id': record.id, 'section_ids': record.section_ids.id, 'sub_payment_id': record.sub_payment_id.id, 'amount': record.amount, 'dependent_type': record.dependent_type}
                dependent_list.append(self.env['section.line.abstract'].create(abs_line).id)
        self.write({'section_line_abstract': [(6, 0, dependent_list)]})
        for sums in self.mapped('summary_line'):
            sums.write({'spouse_cost': 0, 'child_cost': 0})

        if self.section_line_abstract:
            summaryline_id = None
            spousecost = 0
            childcost = 0
            for record in self.mapped('section_line_abstract').filtered(lambda self: self.dependent_type != "child"):
                summarylineids = self.mapped('summary_line').filtered(lambda summ: summ.section_ids.name == record.section_ids.name and summ.sub_payment_id.id == record.sub_payment_id.id)
                if summarylineids:
                    if not summarylineids.one_time_fee:
                        if bill_generate == False:
                            spousecost += record.amount
                            summarylineids.write({'spouse_cost': summarylineids.spouse_cost + record.amount})
                else:
                    spousecost += record.amount
                    summary_line_id = summary_line_obj.create({
                        'section_line_id': record.section_line_id.id,
                        'sub_payment_id': record.sub_payment_id.id,
                        'section_ids': record.section_ids.id,
                        'member_cost': 0,
                        'spouse_cost': record.amount,
                        'child_cost': 0,
                        'spouse_initial_amount': record.amount, 
                        'subscription_period': self.subscription_period,
                    })
                    summaryline_id = summary_line_id.id
                    self.summary_line = [(4, summary_line_id.id)]
                # self.env['summary.section.line'].browse([summaryline_id])
            ...
            for record in self.mapped('section_line_abstract').filtered(lambda self: self.dependent_type == "child"):
                ...
                if summarylineids:
                    if not summarylineids.one_time_fee:
                        if bill_generate == False:
                            childcost += record.amount
                            summarylineids.write({'child_cost': summarylineids.child_cost + record.amount})
                            # summaryline_id = summarylineids[0].id # .update({'spouse_cost': record.amount, 'child_cost': 0, 'member_cost': 0,})
                else:
                    childcost += record.amount
                    summary_line_id = summary_line_obj.create({
                        'section_line_id': record.section_line_id.id,
                        ...
                        'member_cost': 0,
                        'spouse_cost': 0,
                        'child_cost': record.amount,
                        'child_initial_amount': record.amount, 
                        'subscription_period': self.subscription_period,
                    })
                    summaryline_id = summary_line_id.id
                    self.summary_line = [(4, summary_line_id.id)]
    ...
    def calculate_overall_bill(self, period, bill_generate=False):
        """Args: bill_generate: if false, when you hits this method, it will only update the summary section
        ...
        if member_subscription_line:
            for rec in member_subscription_line:
                summary_lines = self.mapped('summary_line').filtered(lambda s: s.section_ids.name == rec.section_ids.name and s.sub_payment_id.id == rec.sub_payment_id.id)
                section_amount = self.env['section.line'].browse([rec.id]) 
                if not summary_lines:
                    summary_line_id = summary_line_obj.create({
                        'section_line_id': rec.id,
                        'sub_payment_id': rec.sub_payment_id.id,
                        'section_ids': rec.section_ids.id,
                        'subscription_period': self.subscription_period,
                        'member_cost': rec.amount,
                        'member_initial_amount': rec.amount,
                    })
                    ...
                    for check in summary_lines:
                        if bill_generate == False:
                            check.write({ 
                                    'member_cost': rec.amount if check.member_cost <= 0 else check.member_cost,
                                    'member_initial_amount': rec.amount, 
                                })
        if member_levy_line:
            for rec in member_levy_line:
                self.generate_member_summary_line(rec.id, subscription_period, rec.sub_payment_id.id, rec.section_ids.id,rec.section_ids.name, rec.amount,bill_generate)
        if member_entry_line:
            for rec in member_entry_line:
                self.generate_member_summary_line(rec.id, subscription_period, rec.sub_payment_id.id, rec.section_ids.id, rec.section_ids.name, rec.amount,bill_generate)
        
        if member_added_line:
            for rec in member_added_line: 
                self.generate_member_summary_line(rec.id, subscription_period, rec.sub_payment_id.id, rec.section_ids.id, rec.section_ids.name, rec.amount,bill_generate)
 
        self.update_dependant_line(bill_generate)                
        ...
        #         self.summary_line_func(mem.id, subscription_period, mem.sub_payment_id.id, mem.section_ids.id, mem.section_ids.name, mem.amount, 0, 0,bill_generate)
...
#  TREAT IMPORTANT REGISTER WHITE
    ...
    def button_white_payments(self):  #  state draft
        ...
        if not self.is_existing:
            ...
            part = partner.create({'street': self.street, 'email': self.email, 'state_id': self.state_id.id,
                                    ...
                                    'name': str(self.surname) +' '+ str(self.first_name) +' '+ middlename,
        ...
        if section_lines:
            for record in section_lines:
                ...
                curr_invoice_line = {
                        ...
                        'price_unit': record_amount.total,
                        'quantity': 1.0,
                    ...
                    for sub2 in section_lines:
                        ...
                        if sub2.is_child != True:
                            if sub2.sub_payment_id.paytype == "main_house" and child.mode in ["jun", "new"]:
                                discount = percentage_cut * amt
                                # total = (discount / 6) * self.number_period if self.duration_period == "Months" else (sub2.price_mainhouse * 2) * self.number_period 
                                total = (discount / 6) * self.number_period if self.duration_period == "Months" else (discount * 2) * self.number_period 
                                child_total += total
                                ...
                                total = (amt / 6) * self.number_period if self.duration_period == "Months" else (amt * 2) * self.number_period 
                                child_total += total
                        else:
                            total = 0
                            child_total += 0
                    ...
                    for sub2 in section_lines:
                        ...
                        if sub2.special_subscription != True:
                            if sub2.sub_payment_id.paytype == "main_house" and spouse.mode in ["jun", "new"]:
                                discount = percentage_cut * amt
                                spousetotal = (discount / 6) * self.number_period if self.duration_period == "Months" else (discount * 2) * self.number_period 
                                spouse_total += spousetotal

                            else: # sub2.sub_payment_id.paytype == "main_house" and not child.mode in ["jun", "new"]:
                                spousetotal = (amt / 6) * self.number_period if self.duration_period == "Months" else (amt * 2) * self.number_period 
                                spouse_total += spousetotal
                        else:
                            spousetotal = amt
                            spouse_total += spousetotal 
        ...
        for inv in self:
            ...
            if self.state == 'white':
                ...
                if not section_lines:
                    raise ValidationError("At the state, Please ensure a Section Line contains an Addtitional Fee Type such as:\n\
                        Coffee, Magazine, Harmony fee, Penalty fee etc.")
                self.define_invoice_line(invoice)
                
            elif self.state == 'white_form_issued':
                section_lines = self.mapped('section_line').filtered(lambda self: self.sub_payment_id.paytype in ['addition', 'others'])
                if not section_lines:
                    raise ValidationError("Please ensure a section line contains an addtitional or penalty fee type such as:\n\
                        Penalty fee, etc.") 
                self.define_invoice_line(invoice)
                ...
                if not section_lines:
                    raise ValidationError("Please ensure a section line contains an addtitional fee type such as:\n\
                        Penalty fee, Green Card Penalty Fee etc.") 
                self.define_invoice_line(invoice)

            elif self.state == 'green':

                section_mainhouse_lines = self.mapped('section_line').filtered(lambda self: self.sub_payment_id.paytype in ['main_house'])
                section_lines = self.mapped('section_line').filtered(lambda self: self.sub_payment_id.paytype in ['entry_fee', 'subscription', 'levy', 'others'])
                if not section_lines:
                    raise ValidationError("Please ensure a section line contains fee of type such as:\n\
                        Subscription fee, Levy, Entry fee etc.")
                elif not section_mainhouse_lines:
                    raise ValidationError("Section line must contain a Main House Type")
    ...
    def button_confirm_white_payments_first(self): 
        ...
        return self.create_member_bill() # self.button_payments(name, amount, level)
    ...
    def _check_fields(self):
        ...
        if not self.section_line:
            errors.append('- Section Line(s)')
 
        if not self.duration_period:
            errors.append('- Duration to Pay e.g Month ie. 6 months or Year -i.e 1 Year')
...
class SummarySectionLine(models.Model):
    ...
    section_ids = fields.Many2one('section.product', string="Sections", ondelete='cascade')
    sub_payment_id = fields.Many2one('subscription.payment', string="Fee", ondelete='cascade')
    spouse_cost = fields.Float('Spouse Cost')     
    child_cost = fields.Float('Child Cost')
    number_of_period = fields.Float('No. of Period', compute="compute_number_of_period")  
    member_cost = fields.Float('Member Cost')     
    one_time_fee = fields.Boolean('One time fee')
    total =  fields.Float('Total Cost', compute="summary_line_total", store=True) 

    member_initial_amount = fields.Float('Member Cost', help="Used to store the computation of member's section amount")   
    spouse_initial_amount = fields.Float('Member Cost', help="Used to store the computation of spouse's section amount")   
    child_initial_amount = fields.Float('Member Cost', help="Used to store the computation of child's section amount")   

    section_line_id = fields.Many2one('section.line', ondelete='cascade')
    subscription_period = fields.Selection([
        ('Jan-June 2011', 'Jan-June 2011'),
        ('July-Dec 2011', 'July-Dec 2011'),
        ('Jan-June 2012', 'Jan-June 2012'),
    ...
    period_payment_line = fields.One2many('period.payment', 'summary_line_id', string="Subscribed Period")

    @api.one
    @api.depends('period_payment_line')
    def compute_number_of_period(self):
        if self.period_payment_line:
            self.number_of_period = len(self.mapped('period_payment_line'))

    @api.one
    @api.depends('spouse_cost', 'child_cost', 'member_cost')
    def summary_line_total(self):
        for rec in self:
            rec.total = sum([rec.spouse_cost, rec.child_cost, rec.member_cost])
...
class Periodpayment(models.Model):
    _name = "period.payment"

    summary_line_id = fields.Many2one('summary.section.line', 'Section line', ondelete='cascade')
    mark_paid = fields.Boolean('Paid')
    date_paid = fields.Date('Date', required=False)
    subscription_period = fields.Selection([
        ('Jan-June 2011', 'Jan-June 2011'),
        ('July-Dec 2011', 'July-Dec 2011'),
        ('Jan-June 2012', 'Jan-June 2012'),
...
class App_subscription_Line(models.Model):
    ...
    section_line = fields.One2many('section.line', "sub_payment_id", string="Section Lines", ondelete='cascade')
...
class RegisterSpouseMember(models.Model):
    ...
    def create_outstanding_line(self, inv_id):
        ...
        income_account = self.env['account.account'].search([('user_type_id','=',1 )], limit=1)
        # balance = members_search.balance_total
        balance = members_search.get_amount_to_pay()
        if balance > 0:
            curr_invoice_subs = {
                                'name': "Outstanding Computed from Payment & Renewal lines", 
                                'price_unit': balance, 
