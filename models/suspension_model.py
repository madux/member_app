...
class Suspend_Member(models.Model):
    ...
    def check_suspended(self):
        ...
        if member:
            if member.activity in ['suspension', 'asuspension']: #or member.state == "suspension":
                raise ValidationError('Member is either Self suspension / Club suspensioned member')
    # @api.constrains('partner_id')
    # def check_suspended(self):
    #     self.ensure_one()
    #     member = self.env['member.app'].search(
    ...
    member_id = fields.Many2one('member.app', 'Member Name', domain=[('activity','!=', ['suspension','asuspension'])], readonly=False, compute="Domain_Member_Field")
    ...
    def send_manager_to_approve(self):
        ...
        if memberx:
            for rec in memberx:
                if self.mode == 'no_self_suspension':
                    rec.write({'activity':'asuspension'})
                    # rec.write({'state': "suspension", 'activity':'inact'})
                else:
                    rec.write({'activity':'suspension'})

            self.write({'suspension_date':fields.Datetime.now()})
            
            spouse = self.env['register.spouse.member'].search([('sponsor','=',self.member_id.id)])
            for spouses in spouse:
                spouses.write({'active':False})
            
            self.send_mail_suspension()
        else:
