...
class subscription_LineMain(models.Model):
    ...
    residual = fields.Float(
        ...
        required=False, help="Remain amount after advance - balance", compute="_get_residual")
    paid_amount = fields.Float(string='Paid Amount', store=True)
    outstanding = fields.Float(string='Balance', store=True)
    total_bill = fields.Float(string='Total Bill', store=True)
    section_line_id = fields.Many2many('section.line', string='Section line')
    mark_paid = fields.Boolean('Paid')
    advance_amount = fields.Float(string='Advance Amount', store=True)
    
    @api.one 
    @api.depends('outstanding', 'advance_amount')
    def _get_residual(self):
        for rec in self:
            if rec.outstanding or rec.advance_amount:
                rec.residual = rec.advance_amount - rec.outstanding # (5000 - 1000 = 5000, )
